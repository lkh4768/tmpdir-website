node {
	stage("Init") {
		...
		imageName = "tmpdir-${packageName[0].toLowerCase()}:${packageVersion}"
		containerName = "tmpdir-${packageName[0].toLowerCase()}-${packageVersion}"
	...
	stage("Deploy on stage") {
		if(env.BRANCH_NAME == developBranchName){
			image = docker.build("${imageName}", "--build-arg PACKAGE_NAME=${packageName} PACKAGE_VERSION=${packageVersion}")
